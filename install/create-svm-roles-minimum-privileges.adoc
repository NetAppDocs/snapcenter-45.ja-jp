...
* 「security login role create -role svm_SVM_Role_Name -cmddirname」「snapmirror list -destinations」-vserver svm_name -access all」を指定します
...
* 「security login role create -vserver svm_name -role SVM_SVM_SVM_SVM_SVM_Role_Name -cmddirname "LUN"-access all」に設定されています
...
* 「security login role create -vserver SVM_name -role SVM_name_Role_Name -cmddirname」vserver export-policy show「-access all `*'security login role create -vserver SVM_name -role SVM_SVM_Role_Name -cmddirname」vserver iscsi connection show「-access all
